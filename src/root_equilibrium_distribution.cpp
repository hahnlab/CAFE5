...
poisson_distribution::poisson_distribution(clade *p_tree, std::vector<gene_family> *p_gene_families)
{
    vector<double> root_poisson_lambda = find_poisson_lambda(p_tree, *p_gene_families);
...
root_equilibrium_distribution* root_eq_dist_factory(const input_parameters& my_input_parameters, clade *p_tree, std::vector<gene_family> *p_gene_families)
{
    ...
    else
    {
        ...
        if (pl > 0)
            ...
            p_prior = new poisson_distribution(p_tree, p_gene_families);
    }
